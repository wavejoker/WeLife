...
public class ProductServiceImpl implements ProductService {
	...
	public ProductInfo getProductById(Long ProductId) {
		try {
			// 调用rest的服务查询商品信息
			String requetJson = HttpClientUtil.doGet(REST_BASE_URL + PRODUCT_DESC_URL + ProductId);
	...
	public String getProductDesc(Long ProductId) {
		try {
			...
			// 转换成Java对象
			WeLifeResult taotaoResult = WeLifeResult.formatToPojo(string, WeProductDesc.class);
			if (taotaoResult.getStatus() == 200) {
				WeProductDesc productDesc = (WeProductDesc) taotaoResult.getData();
